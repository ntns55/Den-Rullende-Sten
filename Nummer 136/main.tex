...
\definecolor{FrontRed}{HTML}{781B1B}
\usepackage{tikz}
...
% Smooth transition used behind cover text: transparent above, dark below.
\tikzfading[
  name=front text fade,
...
\newcommand{\FrontFeatureImage}[5]{%
  \par\noindent
  % The masthead is deliberately compact, so the lead image can use most of
  % the remaining A5 page instead of leaving a large unused area below it.
  \setlength{\FrontMaxHeight}{.88\textheight}%
  \setlength{\FrontWidth}{\dimexpr\textwidth+14mm\relax}%
  % Scale consistently to the same width used by the frame and overlays.
  \sbox{\FrontImgBox}{%
    \includegraphics[width=\FrontWidth]{#1}%
  }%
  \setlength{\FrontClip}{\dimexpr\ht\FrontImgBox+\dp\FrontImgBox\relax}%
  \ifdim\FrontClip>\FrontMaxHeight
    \setlength{\FrontClip}{\FrontMaxHeight}%
  \fi%
  \makebox[\textwidth][c]{\begin{tikzpicture}
    % Positive coordinates give TeX a box with the full visible height;
    % negative coordinates would otherwise let the picture overlap the masthead.
    \path[use as bounding box] (0,0) rectangle (\FrontWidth,\FrontClip);
    \begin{scope}
      \clip (0,0) rectangle (\FrontWidth,\FrontClip);
      \node[anchor=north west,inner sep=0pt] at (0,\FrontClip)
        {\usebox{\FrontImgBox}};

      % Fade gently into black instead of drawing a visible rectangular box.
      \fill[black,path fading=front text fade,fit fading=false]
        (0,0) rectangle (\FrontWidth,.55\FrontClip);
    \end{scope}

    % Hairline frame: enough definition for print, without boxing in the art.
    \draw[black!65,line width=.45pt]
      (0,0) rectangle (\FrontWidth,\FrontClip);
    ...
    \node[
      ...
      yshift=.35em,
      align=left,
      text width=\dimexpr\FrontWidth-2em\relax
    ] at (0,0) {%
      \sffamily
      \if\relax\detokenize{#2}\relax\else
        {\bfseries\small\color{white}%
          \colorbox{FrontRed}{\strut\hspace{.35em}#2\hspace{.35em}}}%
        \par\smallskip
      \fi
      {\bfseries\color{white}\resizebox{\linewidth}{!}{#3}}\par
      {\color{FrontRed}\rule{\linewidth}{1.1pt}}\par\smallskip
      {\large\color{white}#4}%
    };
    ...
    \if\relax\detokenize{#5}\relax\else
      \node[
        ...
        xshift=-.5em,
        yshift=-.5em
      ] at (\FrontWidth,\FrontClip) {\sffamily\scriptsize\color{white}#5};
  \fi
  \end{tikzpicture}}%
  \par
}

% A compact issue line gives the masthead a repeatable newspaper structure.
\newcommand{\FrontIssueLine}[1]{%
  \par\smallskip
  \noindent
  \makebox[\textwidth]{%
    {\color{FrontRed}\rule[.55ex]{.32\textwidth}{.7pt}}%
    \hfill{\sffamily\bfseries\small #1}\hfill
    {\color{FrontRed}\rule[.55ex]{.32\textwidth}{.7pt}}%
  }%
  \par\medskip
}
